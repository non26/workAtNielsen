...
from PyNielsen.kim.nonEdit import settingUp
...
class UIForAutoCopy(tk.Tk):
    ...
    def copy(self):
        ...
        if check.find("scantrack") != -1:
            self.autoCp.autocopy_everyLevel()
        else:
            self.autoCp.autoCopy_oneLevel()

    def checkSize(self):
        if type(self.autoCp) is None: pass
        else: self.autoCp.SizeCheckingFromG()
